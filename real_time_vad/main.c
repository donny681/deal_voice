...
#define LENGTH 12  //录音时间,秒
#define RATE 8000  //采样频率
#define SIZE 16    //量化位数
#define CHANNELS 1 //声道数目
#define RSIZE (9600*2)   //buf的大小
#define per_ms_frames 30
size_t samples = 0;
size_t nTotal = 0;
 void *vadInst =NULL;
/********以下是wave格式文件的文件头格式说明******/
...
{
    int vad_mode = 3;
    samples = RATE * per_ms_frames / 1000;
    nTotal = (9600 / samples);
...
int main()
{
    ...
    printf("size = %d\n", size);
    buffer = (uint16_t *)malloc(sizeof(uint16_t)*frames);
    int mysize=(sizeof(uint16_t))*frames;
    /* We want to loop for 5 seconds */
    snd_pcm_hw_params_get_period_time(params, &val, &dir);
    int len = 0;
    // uint16_t *ram_buffer = (uint16_t *)malloc(9600);
    // memset(ram_buffer, 0, 9600);
    // uint16_t *mypointer = NULL;
    // mypointer = ram_buffer;
    webrtc_vad_init();
    uint16_t *ram_buffer=NULL;
    for (i = 1; i <= (LENGTH * RATE * SIZE * CHANNELS / 8) / RSIZE ; i++)
    {

        rc = snd_pcm_readi(handle, buffer, frames);
        printf("%d,%d\n", i,(LENGTH * RATE * SIZE * CHANNELS / 8) / RSIZE);
        if (rc == -EPIPE)
        {
            /* EPIPE means overrun */
        ...
        {
            fprintf(stderr, "read rc=%d,len=%d len\r\n", rc, len = len + rc);
        }

        if (write(fd_f, buffer, mysize) == -1)
        {
            perror("writlse to sound wrong!!");
        }
        ...
        else
        {
            ...
            {
                int keep_weight = 0;
                int nVadRet = WebRtcVad_Process(vadInst, RATE, ram_buffer, samples, keep_weight);
                if (nVadRet == -1)
                {
